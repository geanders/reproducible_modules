...
from measurement, to data recording, pre-processing, analysis, and interpretation.  \textbf{However, in practice the distinct actions of data recording, data pre-processing, and analysis 
...
system / software combination (Figure \ref{fig:workflow}), resulting in key failure points for reproducibility at the stages of data recording and pre-processing.}  
...
transform, and reformat various elements of the data shared with them by laboratory-based researchers, and that data is often shared in an unstructured format, increasing the risks of introducing errors through reformatting the data to apply more advanced computational methods
...
critical points of failure for reproducible research. Red arrows indicate where data is passed to other research team members, including statisticians / data analysts, often within complex or unstructured spreadsheet files.} 
...
used to extend a transparent and reproducible data workflow to data recording and pre-processing. 
...
``Improving the Reproducibility of Experimental Data Recording" (Table
...
sequence, ``Improving the Reproducibility of Experimental Data
...
attract early users to help disseminate the material as it evolves and also to get early feedback.} Once the online
...
biomedical laboratory-based research programs, including those at Colorado
State University. Our project's \textbf{primary goal} is to develop
...
The training modules will be divided into two sequences, ``Improving the Reproducibility of Experimental Data
Recording" and ``Improving the Reproducibility of Experimental Data
Pre-Processing".
...
point-and-click software is often available for this pre-processing, it is used interactively and often does not create a history of steps and choices \cite{pernet2015improving}, at least not in a format that is easy for a statistician to navigate, understand, and check \cite{peng2011reproducible, pernet2015improving}. 
...
audio content through SoundCloud \cite{soundcloud}, which will allow us embed the hosted content in the online book. We will use Google Forms \cite{googleforms} as a free and unlimited way
...
version control tools and platforms can enable ``live" development of a project. 
...
We will use Google Forms \cite{googleforms} to create voluntary surveys within the online book, to generate additional feedback from these early users in terms of their characteristics and on the usefulness of the modules.  While this voluntary survey will not provide data on \textit{all} early online users, it will help characterize a collection of the users, and we selected to make the survey voluntary (rather than required to access the book) in hopes of making the book as accessible as possible, and to avoid discouraging users who might be drawn in by browsing the materials before committing to provide their information or feedback.
...
\begin{figure}[ht]
    ...
    \caption{Timeline for proposed activities for this project. The solid red lines show the start and end of the project period, while the dotted red lines separate each project year. All training materials will be freely and publicly available online by the end of the second project year.}
    \label{fig:timeline}
\end{figure}
